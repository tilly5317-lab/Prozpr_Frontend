...
// ── Additional investment: monthly SIP plan (Invest page) ───────────────
// The plan is generated by the additional-investment engine inside chat and
// read back here for the Invest page. `has_plan` is false until the user sets
// one up (e.g. "start a ₹25k monthly SIP").

/** One fund in the SIP plan with its per-month contribution. */
export interface SipFundBuy {
...
/** Latest monthly-SIP deployment plan — mirrors backend `SipPlanResponse`. */
export interface SipPlanResponse {
  has_plan: boolean;
  run_id: string | null;
  created_at: string | null;
  /** Total fresh money the plan deploys each month. */
  monthly_amount_inr: number;
  /** Of the monthly amount: how much lands in funds vs. couldn't be placed. */
  monthly_deployed_inr: number;
  monthly_undeployed_inr: number;
  /** Horizon the SIP leans toward; null when there is no plan. */
...
/**
 * The user's latest monthly-SIP plan (from the additional-investment engine).
 * `has_plan` is false when they haven't set one up yet — render the set-up
 * prompt rather than a plan.
 */
export async function getMySipPlan(): Promise<SipPlanResponse> {
...
export async function createSipPlan(monthlyAmountInr: number): Promise<SipPlanResponse> {
  return request<SipPlanResponse>("/additional-investment/sip", {
    method: "POST",
    body: JSON.stringify({ monthly_amount_inr: monthlyAmountInr }),
  });
}
